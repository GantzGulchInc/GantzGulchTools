...
public class GGDigestScheme extends DigestScheme {
    ...
    public GGDigestScheme(Charset credentialsCharset) {
    ...
    /** @deprecated */
    @Deprecated
    public GGDigestScheme(ChallengeState challengeState) {
        super(challengeState);
    }

    public GGDigestScheme() {
        this(Consts.ASCII);
    }
    ...
    public Header authenticate(Credentials credentials, HttpRequest request, HttpContext context) throws AuthenticationException {
        ...
        if (this.getParameter("realm") == null) {
            ...
            this.getParameters().put("uri", getPath(request));
    ...
    private String getPath(final HttpRequest request) {
        final String uriText = request.getRequestLine().getUri();
    ...
    private Header createDigestHeader(Credentials credentials, HttpRequest request) throws AuthenticationException {
        ...
        if (qoplist != null) {
            ...
            while(tok.hasMoreTokens()) {
                digAlg = tok.nextToken().trim();
                qopset.add(digAlg.toLowerCase(Locale.ROOT));
            }
        ...
        if (qop == -1) {
            ...
            if (algorithm.equalsIgnoreCase("MD5-sess")) {
                ...
                sb.setLength(0);
                sb.append(hasha1).append(':').append(nonce).append(':').append(this.cnonce);
                this.a1 = sb.toString();
            } else {
                sb.setLength(0);
                sb.append(uname).append(':').append(realm).append(':').append(pwd);
            ...
            hasha1 = encode(digester.digest(EncodingUtils.getBytes(this.a1, charset)));
            if (qop == 2) {
                this.a2 = method + ':' + uri;
            } else if (qop == 1) {
